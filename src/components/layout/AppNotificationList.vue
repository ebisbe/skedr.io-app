<template>
  <v-navigation-drawer v-model="rightDrawer" fixed temporary right>
    <ApolloQuery :query="require('@/graphql/notifications.gql')" :variables="{ count }" tag="">
      <template slot-scope="{ result: { error, data }, isLoading, query, gqlError }">
        <v-container fluid pa-0>
          ...
          <template v-else-if="data && data.notifications.notifications.length">
            <v-layout v-if="refetchData" py-2 px-4>
              <v-flex>
                <v-btn
                  v-t="'btn.update'"
                  :loading="isLoading === 1"
            ...
            <v-list three-line>
              <template v-for="(item, index) in data.notifications.notifications">
                <v-divider v-if="index !== 0" :key="`${index}-divider`" />
                <v-list-item :key="index" @click.stop>
                  <v-list-item-avatar>
                  ...
                  <v-list-item-content>
                    ...
                    <v-list-item-subtitle
                      >{{ item.group.title }} <br />
                      {{ createdAt(item.createdAt) }}</v-list-item-subtitle
                    >
                  </v-list-item-content>
                </v-list-item>
              </template>
            </v-list>
            <v-layout v-if="showMoreEnabled" py-2 px-4>
              <v-flex>
                <v-btn
                  v-t="'btn.load_more'"
                  :loading="isLoading === 1"
    ...
    methods: {
      ...
      showMore(query, nextToken) {
        ...
        query.fetchMore({
          ...
          updateQuery: (
            { notifications: { notifications: prevNot } },
