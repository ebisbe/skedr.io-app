  ...
  <v-content>
    ...
    >
      <template slot-scope="{ result: { data, loading, error }, query }">
        ...
        <!-- Error -->
        <q-empty v-else-if="error" :error="true" :description="error" icon="access_time" />
        ...
        >
          <v-layout
            ...
            class="pb-4"
          >
            <v-flex xs2 sm1 class="headline">
              <span class="date">{{ item[0].headerDate }}</span>
            </v-flex>
            <v-flex xs10 sm11 class="pt-0">
              <template v-for="(group, title) in groups(item)">
                <h2 v-if="group" :key="title + index" class="title">
                  {{ `${group[0].group.title} (${group.length})` }}
                </h2>
                <v-layout :key="title + index + 'layout'" row wrap>
                  <v-flex
          ...
          <v-flex v-if="showMoreEnabled" xs12>
            ...
            >
              ...
              <span v-else> &nbsp;{{ $t('btn.load_future_dates') }} </span>
            </v-btn>
          </v-flex>
        </v-container>
        <q-empty v-else :description="$t('Layout.no_photos_scheduled')" icon="access_time" />
      </template>
    </ApolloQuery>
  </v-content>
</template>
